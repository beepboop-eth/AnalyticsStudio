...
import ReactMenuAim from '../libs/reactMenuAim.js';
...
var Menu = React.createClass({
  ...
  render: function() {
    ...
    return (
      <div className={containerClassName} id = "menoo">
        ...
        <ul className="sub-menu" style={subMenuStyle}>
          {this.props.menuData[this.state.activeMenuIndex].subMenu.map((function(subMenu, index){
            return (
              <li className="sub-menu-item" onClick={function() {if(index === 0) prompt('Create new file');}} key={index}>{subMenu}</li>
            );
          }))}
        </ul>
      </div>
